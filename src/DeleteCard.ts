...
export class DeleteCard {
   ...
   */
  delete(toDeleteId: number, callback: (error: string | undefined, data: string | undefined) => void): void {
    ...
    if (found) {
      rm(`${this.Cards.getUser()}/${found.name}.json`, (err) => {
        ...
        else callback(undefined, `The card ${toDeleteId} has been deleted from the collection of ${this.Cards.getUser()}`); 
      });
    }
