...
export class ListCards {
   ...
   */
  list(callback: (error: string | undefined, data: Card[] | undefined) => void): void {
    if (this.Cards.collection.length === 0) callback("No hay elementos en la colección", undefined);
    else {
      const cardList: Card[] = [];
