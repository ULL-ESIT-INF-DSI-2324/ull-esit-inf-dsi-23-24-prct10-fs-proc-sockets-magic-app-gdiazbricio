...
export class ModifyCard {
   ...
   */
  modify(toModify: Card, callback: (error: string | undefined, data: string | undefined) => void): void {
    ...
    if (found) {
      ...
      rename(urlPath, `${this.Cards.getUser()}/${toModify.name}.json`, (err) => {
        if (err) callback("Error en el servidor", undefined);
        else callback(undefined, `The card ${toModify.id} has been modified in the collection of ${this.Cards.getUser()}`);
      });
    }
    else {
